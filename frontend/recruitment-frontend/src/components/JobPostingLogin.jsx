    ...
    if (isLogin) {
      ...
      api.login({ email: formData.email, password: formData.password, role: 'recruiter' })
        .then(data => {
            ...
            // Store auth data from correct response structure
            ...
            if (data.data && data.data.user && data.data.user.role) {
              localStorage.setItem('authRole', data.data.user.role);
              localStorage.setItem('authUserName', data.data.user.name);
              localStorage.setItem('authUserEmail', data.data.user.email);
            ...
            console.log('🔐 Login successful - Role stored:', localStorage.getItem('authRole'));
            
            // Remove any alerts that might interfere
            // alert('Login successful!');
            
            // Force immediate navigation
            setTimeout(() => {
              console.log('🔀 Navigating to jobposting-dashboard...');
      ...
      api.register({
        ...
        role: 'recruiter', // This is correct based on validation 'in:recruiter,job_seeker'
        ...
        .then(data => {
            ...
            console.log('🔐 Auth stored - Token:', localStorage.getItem('authToken'));
            console.log('🔐 Auth stored - Role:', localStorage.getItem('authRole'));
