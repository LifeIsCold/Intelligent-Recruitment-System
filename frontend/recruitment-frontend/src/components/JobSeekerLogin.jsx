...
const JobSeekerLogin = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (isLogin) {
      ...
      api.login({ email: formData.email.trim(), password: formData.password, role: 'job_seeker' })
        .then(data => {
          ...
          if (user) {
            localStorage.setItem('authRole', user.role || 'job_seeker');
            localStorage.setItem('authUserName', user.name || '');
            localStorage.setItem('authUserEmail', user.email || '');
            
            // 👇 ADD THIS - Store profile picture
            if (user.profile_picture_url) {
              localStorage.setItem('userProfilePicture', user.profile_picture_url);
            }
          }
